...
@export var initial_event_id: String = "sword_event"
...
func _ready() -> void:
    _current_event = EventFactory.get_builder(initial_event_id).build(_world_state)
...
func resolve_event(choice: String) -> void:
    ...
    if next_event_id == null:
        ...
        _current_event = EventFactory.get_builder(next_event_id).build(_world_state)
