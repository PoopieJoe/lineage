...
func _on_event_resolved(new_event: EventRootNode) -> void:
	var elements = EventUIBuilder.make_UIElements(new_event, storyteller.resolve_event)
	add_section(elements)
	next_page()

func _ready() -> void:
	if $DebugLog:
		Logger.set_output($DebugLog)
	...
	var elements = EventUIBuilder.make_UIElements(event, storyteller.resolve_event)
